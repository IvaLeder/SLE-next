    ...
    <>
      ...
      {visible.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((post, i) => (
            // Only the first card can become the mobile LCP. Preloading three
            // full-width covers made the others compete with critical assets.
            <PostCard key={post.slug} post={post} lang={lang} priority={i === 0} headingLevel="h2" />
          ))}
        </div>
      )}
